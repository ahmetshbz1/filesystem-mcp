...
async function searchInFile(filePath, query, useRegex, caseSensitive, contextLines) {
    try {
...
export const tools = [
    ...
    },
    {
        ...
        inputSchema: zodToJsonSchema(ContentSearchArgsSchema)
    },
    {
        name: 'fuzzy_search',
        description: 'Fuzzy search for files by name similarity',
        inputSchema: zodToJsonSchema(FuzzySearchArgsSchema)
    },
];
export const handlers = {
    ...
    async content_search(args) {
        ...
        if (!parsed.success)
            throw new Error(`Invalid arguments for content_search: ${parsed.error}`);
    ...
    async fuzzy_search(args) {
        ...
        if (!parsed.success)
            throw new Error(`Invalid arguments for fuzzy_search: ${parsed.error}`);
        ...
        const output = topResults.map((r) => `${(r.score * 100).toFixed(1)}% - ${r.path}`).join('\n');

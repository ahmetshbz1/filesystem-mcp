...
async function validateJavaScript(filePath) {
    try {
        ...
        const { stdout, stderr } = await execAsync(`node --check ${filePath}`, { maxBuffer: 10 * 1024 * 1024 });
        if (!stderr)
            return [];
        const errors = [];
...
async function runESLint(filePath, fix, configPath) {
    try {
        const fixFlag = fix ? '--fix' : '';
        const configFlag = configPath ? `--config ${configPath}` : '';
        const { stdout } = await execAsync(`npx eslint ${fixFlag} ${configFlag} --format stylish ${filePath}`, { maxBuffer: 10 * 1024 * 1024 });
...
export const tools = [
    {
        ...
        inputSchema: zodToJsonSchema(SyntaxCheckArgsSchema)
    },
    {
        name: 'lint_file',
        description: 'Run ESLint on a file with optional auto-fix',
        inputSchema: zodToJsonSchema(LintArgsSchema)
    }
];
export const handlers = {
    async syntax_check(args) {
        ...
        if (!parsed.success)
            throw new Error(`Invalid arguments for syntax_check: ${parsed.error}`);
    ...
    async lint_file(args) {
        ...
        if (!parsed.success)
            throw new Error(`Invalid arguments for lint_file: ${parsed.error}`);

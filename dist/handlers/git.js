...
async function runGitCommand(cwd, command) {
    try {
...
export const tools = [
    ...
    },
    {
        ...
        inputSchema: zodToJsonSchema(GitBranchListArgsSchema)
    },
    {
        name: 'git_show',
        description: 'Show git commit details',
        inputSchema: zodToJsonSchema(GitShowArgsSchema)
    },
    {
        name: 'git_blame',
        description: 'Show git blame for a file',
        inputSchema: zodToJsonSchema(GitBlameArgsSchema)
    }
];
export const handlers = {
    ...
    async git_diff(args) {
        ...
        const output = await runGitCommand(validPath, `git diff ${stagedFlag} --unified=${unified} ${fileArg}`);
    ...
    async git_branch_list(args) {
        ...
        if (!parsed.success)
            throw new Error(`Invalid arguments for git_branch_list: ${parsed.error}`);
    ...
    async git_show(args) {
        ...
        if (!parsed.success)
            throw new Error(`Invalid arguments for git_show: ${parsed.error}`);
    ...
    async git_blame(args) {
        ...
        if (!parsed.success)
            throw new Error(`Invalid arguments for git_blame: ${parsed.error}`);

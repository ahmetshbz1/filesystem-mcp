...
- **read_text_file**
  ...
  - Inputs:
    ...
    - `stream` (boolean, optional): Stream large files in chunks (default: false)
...
- **write_file**
  - Create new file, overwrite existing, or append to existing
  - Inputs:
    - `path` (string): File location
    - `content` (string): File content
    - `append` (boolean, optional): If true, append to file instead of overwriting (default: false)
...
- **edit_file**
  ...
  - Inputs:
    ...
    - `edits` (array): List of edit operations
      - `oldText` (string): Text to search for (can be substring or regex pattern)
      - `newText` (string): Text to replace with
      - `useRegex` (boolean, optional): If true, treat oldText as regex pattern
...
- **list_directory_with_sizes**
  ...
  - Inputs:
    ...
    - `sortBy` (string, optional): Sort entries by "name", "size", "mtime", or "atime" (default: "name")
...
- **search_files**
  ...
  - Inputs:
    ...
    - `fileTypes` (string[], optional): Filter by file extensions (e.g., ['.ts', '.js'])
  - Glob-style pattern matching
  - Returns full paths to matches

- **file_compare**
  - Compare two files and return the differences in unified diff format
  - Inputs:
...
- **file_backup**
  ...
  - Inputs:
    - `path` (string): Path to the file to backup
    - `backupPath` (string, optional): Backup path, defaults to original with .bak extension
  - Creates a copy of the file

- **compress_file**
  - Compress a file using gzip
  - Inputs:
    - `path` (string): Path to the file to compress
    - `outputPath` (string, optional): Output path, defaults to original with .gz extension
...
- **file_hash**
  ...
  - Inputs:
    ...
    - `algorithm` (string, optional): Hash algorithm (md5, sha1, sha256, sha512), default sha256
  - Returns the hash value

- **file_merge**
  - Merge multiple files into one
  - Inputs:
    - `paths` (string[]): Array of file paths to merge (at least 2)
    - `outputPath` (string): Path for the merged file
    - `separator` (string, optional): Separator between files, default newline
  - Combines files in order

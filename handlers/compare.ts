...
async function compareBinaryFiles(path1: string, path2: string): Promise<boolean> {
...
export const tools = [
  {
    ...
    inputSchema: zodToJsonSchema(FileCompareArgsSchema) as ToolInput
  },
  {
    name: 'binary_compare',
    description: 'Compare two binary files byte-by-byte',
    inputSchema: zodToJsonSchema(BinaryCompareArgsSchema) as ToolInput
  },
  {
    name: 'directory_compare',
    description: 'Compare two directories and show differences',
    inputSchema: zodToJsonSchema(DirectoryCompareArgsSchema) as ToolInput
  }
];

export const handlers = {
  file_compare: async (args: z.infer<typeof FileCompareArgsSchema>) => {
  ...
  binary_compare: async (args: z.infer<typeof BinaryCompareArgsSchema>) => {
  ...
  directory_compare: async (args: z.infer<typeof DirectoryCompareArgsSchema>) => {

...
export const tools = [
  {
    ...
    inputSchema: zodToJsonSchema(FileMergeArgsSchema) as ToolInput
  },
  {
    name: 'json_merge',
    description: 'Intelligently merge multiple JSON files',
    inputSchema: zodToJsonSchema(JsonMergeArgsSchema) as ToolInput
  }
];

export const handlers = {
  file_merge: async (args: z.infer<typeof FileMergeArgsSchema>) => {
  ...
  json_merge: async (args: z.infer<typeof JsonMergeArgsSchema>) => {
    ...
    let merged: any = Array.isArray(validObjects[0]) ? [] : {};

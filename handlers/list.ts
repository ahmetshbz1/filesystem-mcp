...
export const tools = [
  { name: 'list_directory', description: 'List directory with pagination, filtering, and hidden file control', inputSchema: zodToJsonSchema(ListDirectoryArgsSchema) as ToolInput },
  { name: 'list_directory_with_sizes', description: 'List directory with sizes, permissions, and sorting', inputSchema: zodToJsonSchema(ListDirectoryWithSizesArgsSchema) as ToolInput },
  { name: 'directory_tree', description: 'Directory tree with depth control, gitignore support, and size info', inputSchema: zodToJsonSchema(DirectoryTreeArgsSchema) as ToolInput },
  { name: 'recursive_list', description: 'Recursively list files with pattern matching and depth control', inputSchema: zodToJsonSchema(RecursiveListArgsSchema) as ToolInput },
];
...
export const handlers: Record<string, (args: any) => Promise<any>> = {
  async list_directory(args) {
    ...
    const formatted = paginatedEntries.map((entry) =>
  ...
  async list_directory_with_sizes(args) {
    ...
    const sortedEntries = [...detailedEntries].sort((a, b) => {
      ...
      if (parsed.data.sortBy === 'mtime') return b.mtime.getTime() - a.mtime.getTime();
      if (parsed.data.sortBy === 'atime') return b.atime.getTime() - a.atime.getTime();
  ...
  async directory_tree(args) {
    ...
    async function buildTree(currentPath: string, depth: number = 0): Promise<any[]> {
  ...
  async recursive_list(args) {
    ...
    if (!parsed.success) throw new Error(`Invalid arguments for recursive_list: ${parsed.error}`);
    ...
    async function traverse(currentPath: string, depth: number = 0): Promise<void> {
      ...
      for (const entry of entries) {
        ...
        const shouldExclude = excludePatterns.some(patternStr =>
          minimatch(relativePath, patternStr, { dot: true })
        );
